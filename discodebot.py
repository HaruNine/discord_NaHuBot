...
ffmpeg_path = r'C:\\Users\\HRuin\\Desktop\\discodebot\\ffmpeg-2025-02-20-git-bc1a3bfd2c-essentials_build\\bin\\ffmpeg.exe'  # 설치된 FFmpeg 실행 파일의 경로 (적절히 변경 필요)

#ffmpeg_path = os.path.join(os.path.dirname(__file__), 'ffmpeg-2025-02-20-git-bc1a3bfd2c-essentials_build', 'bin', 'ffmpeg.exe')
    ...
    async def register_voice_state_listener(self):
        ...
        async def on_voice_state_update(member, before, after):
            ...
            if before.channel is not None and after.channel is None:  # 사용자가 음성 채널에서 나갔을 때
                ...
                # 개인 메시지 전송 및 삭제
                message = await member.send("나후가 음성 채널에서 나가요. 다음에 또 불러주세요!")  # 개인 메시지 전송
            ...
            if after.channel is not None and member != self.guild.me:
                if len(after.channel.members) == 1:  # 사용자가 혼자 있을 경우
                    ...
                    await member.send("나후가 음성 채널에서 나가요. 다음에 또 불러주세요!")  # 개인 메시지 전송
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            if self.queue.empty() and not (self.loop or self.queue_loop):
                await self.delete_messages()
                await self.stop()
                await self.channel.send("선생님, 재생할 곡이 더 이상 없어요. 나후가 음성 채널에서 나갈게요~", delete_after=10)
                break
    ...
    async def create_player_message(self):
        ...
        async def volume_up_callback(interaction):
            if self.volume < 1.0:
                ...
                await interaction.response.send_message("앗, 볼륨이 이미 최대예요! 나후의 귀가 아파요~", ephemeral=True, delete_after=3)
        ...
        async def stop_callback(interaction):
            if self.guild.voice_client.is_playing():
                await self.stop()
                await interaction.response.send_message("알겠습니다, 선생님! 나후가 음악 재생을 종료하고 음성 채널에서 나갔어요~ 다음에 또 불러주세요!", ephemeral=True, delete_after=3)
            else:
        ...
        message = await self.channel.send("선생님, 나후의 특별 음악 컨트롤이에요! 어떤 걸 눌러볼까요?", view=view)
        return message, view
...
class Music(commands.Cog):
    ...
    async def queue(self, ctx):
        ...
        if player.queue.empty():
            await ctx.send("앗, 선생님! 재생 목록이 비어있어요! 노래를 추가해주시면 나후가 열심히 불러드릴게요~", delete_after=10)
            await self.delete_command_message(ctx)
            return

        upcoming = list(player.queue._queue)
        fmt = '\n'.join(f'**`{_['title']}`**' for _ in upcoming)
        embed = discord.Embed(title=f'나후의 재생 목록 - 총 {len(upcoming)}곡이에요!', description=fmt)
    ...
    async def volume(self, ctx, volume: int):
        ...
        if ctx.voice_client is None:
            await ctx.send("어라? 나후가 아직 음성 채널에 없어요. 먼저 들어가 볼게요!", delete_after=10)
            await self.delete_command_message(ctx)
            return
            ...
            async def add_to_queue_callback(add_interaction):
                ...
                if not ctx.voice_client:
                    if ctx.author.voice:
                        ...
                        await add_interaction.response.send_message("선생님, 음성 채널에 먼저 입장해주세요! 나후가 어디로 가야 할지 모르겠어요~", ephemeral=True, delete_after=10)
                        return
    ...
    async def 삭제(self, ctx, amount: int = 2):
        """지정된 수의 메시지를 삭제합니다."""
        if amount < 1:
            return await ctx.send("어라? 1개 이상의 메시지를 지정해 주셔야 해요. 나후가 삭제할 수 있게요!", delete_after=10)

        deleted = await ctx.channel.purge(limit=amount + 1)  # 명령어 메시지도 포함해서 삭제
        await ctx.send(f"선생님, {len(deleted)-1}개의 메시지를 깨끗하게 지웠어요! 나후가 열심히 청소했답니다~", delete_after=5)
        await self.delete_command_message(ctx)

    @삭제.error
    async def 삭제_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.send("앗, 죄송해요 선생님. 이 명령어는 특별한 권한이 필요해요. 나후가 도와드리고 싶어도 못 해드려요...", delete_after=10)
    ...
    async def restart(self, ctx):
        """프로그램을 재시작합니다."""
        await ctx.send("나후가 재시작할게요! 잠시만 기다려주세요...", delete_after=5)  # 수정된 부분
    ...
    async def stop(self, ctx):
            ...
            if ctx.voice_client:  
                ...
                await ctx.send("나후가 음성 채널을 떠날게요!", delete_after=10)
            else:
