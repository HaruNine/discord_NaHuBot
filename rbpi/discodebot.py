...
discord_logger.setLevel(logging.WARNING)  # WARNING 이상만 출력
...
class FuzzyBot(commands.Bot):
    def __init__(self, *args, **kwargs):
        ...
        self.command_queue = asyncio.Queue()  # 명령어 큐 추가
        self.processing_command = False  # 명령어 처리 상태 추가

    async def setup_hook(self):
        self.session = aiohttp.ClientSession()  # 세션 생성
        self.loop.create_task(self.process_commands())  # 명령어 처리 태스크 추가

    async def close(self):
        if self.session:
            await self.session.close()  # 세션 종료
        await super().close()
...
# Remove commands with a single '!' prefix
@bot.check
...
# Dynamically find the ffmpeg path
def get_ffmpeg_path():
    try:
...
class MusicPlayer:
    def __init__(self, ctx):
        ...
        # cache_folder 경로를 동적으로 설정
        # self.cache_folder = os.path.join(os.path.dirname(__file__), 'cache', str(self.guild.id))
        # 캐시 폴더가 존재하지 않으면 생성
    ...
    async def register_voice_state_listener(self):
        ...
        async def on_voice_state_update(member, before, after):
            ...
            if before.channel is not None and after.channel is None:  # 사용자가 음성 채널에서 나갔을 때
                ...
                # 개인 메시지 전송 및 삭제
                message = await member.send("바다가 음성 채널에서 나가요. 다음에 또 불러주세요!")  # 개인 메시지 전송
            ...
            if after.channel is not None and member != self.guild.me:
                if len(after.channel.members) == 1:  # 사용자가 혼자 있을 경우
                    ...
                    await member.send("바다가 음성 채널에서 나가요. 다음에 또 불러주세요!")  # 개인 메시지 전송
                    await self.restart_program()
    
    async def now_playing(self):
        """현재 재생 중인 노래 정보를 반환합니다."""
        if self.current:
            return f"현재 재생 중인 노래: {self.current['title']}"
        else:
            return "현재 재생 중인 노래가 없습니다."
    
    async def play_first_in_queue(self):
        """대기열의 첫 번째 곡을 재생합니다."""
        if self.queue.empty():
            await self.channel.send("대기열이 비어있어요!", delete_after=10)
            return
        ...
        await self.channel.send(f'선생님, 지금 재생 중인 노래예요: {source["title"]}')

    async def on_voice_state_update(self, member, before, after):
        if member == self.guild.me and after.channel is not None:
            # 봇이 음성 채널에 다시 연결되었을 때 대기열의 첫 번째 곡을 재생
            await self.play_first_in_queue()
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            if self.queue.empty() and not (self.loop or self.queue_loop):
                await self.delete_messages()
                await self.stop()
                await self.channel.send("선생님, 재생할 곡이 더 이상 없어요. 바다가 음성 채널에서 나갈게요~", delete_after=10)
                break
    ...
    async def create_player_message(self):
        ...
        async def volume_up_callback(interaction):
            if self.volume < 1.0:
                ...
                await interaction.response.send_message("앗, 볼륨이 이미 최대예요! 바다의 귀가 아파요~", ephemeral=True, delete_after=3)
        ...
        async def stop_callback(interaction):
            if self.guild.voice_client.is_playing():
                await self.stop()
                await interaction.response.send_message("알겠습니다, 선생님! 바다가 음악 재생을 종료하고 음성 채널에서 나갔어요~ 다음에 또 불러주세요!", ephemeral=True, delete_after=3)
            else:
        ...
        message = await self.channel.send("선생님, 바다의 특별 음악 컨트롤이에요! 어떤 걸 눌러볼까요?", view=view)
        return message, view
    ...
    async def play_next(self):
        while True:  # 무한 루프 시작
            ...
            # 현재 재생 중인 노래가 없을 경우 대기열의 첫 번째 곡을 재생
...
class Music(commands.Cog):
    ...
    async def now_playing(self, ctx):
        ...
        # 현재 재생 중인 노래가 없을 경우 대기열의 첫 번째 곡을 재생
        if player.current is None and not player.queue.empty():
            await player.play_first_in_queue()
            now_playing_info = await player.now_playing()
            
        # 큐 상태 가져오기
        queue_list = list(player.queue._queue)
        if queue_list:
            queue_info = "\n".join([f"{i+1}. {item['title'] if isinstance(item, dict) else item}" for i, item in enumerate(queue_list)])
        else:
            queue_info = "대기열이 비어있어요!"

        # 메시지 보내기
        await ctx.send(f"{now_playing_info}\n\n**대기열:**\n{queue_info}", delete_after=30)
    ...
    async def stop(self, ctx):
        ...
        player = self.get_player(ctx)
        await player.stop()
        await ctx.send("선생님, 바다가 음악 재생을 종료하고 음성 채널에서 나갔어요~ 다음에 또 불러주세요!", delete_after=10)
        await self.delete_command_message(ctx)

    @commands.command(name='나가',aliases=['나가기'])
    ...
    #         await ctx.send("앗, 선생님! 재생 목록이 비어있어요! 노래를 추가해주시면 바다가 열심히 불러드릴게요~", delete_after=10)
    #         await self.delete_command_message(ctx)
    #         return

    #     upcoming = list(player.queue._queue)
    #     fmt = '\n'.join(f'**`{{_["title"]}}`**' for _ in upcoming)
    #     embed = discord.Embed(title=f'바다의 재생 목록 - 총 {len(upcoming)}곡이에요!', description=fmt)
    ...
    async def volume(self, ctx, volume: int):
        ...
        if ctx.voice_client is None:
            await ctx.send("어라? 바다가 아직 음성 채널에 없어요. 먼저 들어가 볼게요!", delete_after=10)
            await self.delete_command_message(ctx)
            return
        ...
        if 0 <= volume <= 100:
            ...
            await ctx.send("앗, 볼륨은 0에서 100 사이로 해주세요~ 바다의 귀가 아파요!", delete_after=10)
        await self.delete_command_message(ctx)
                ...
                try:
                    ...
                    if not ctx.voice_client:
                        if ctx.author.voice:
                            ...
                            await add_interaction.response.send_message("선생님, 음성 채널에 먼저 입장해주세요! 바다가 어디로 가야 할지 모르겠어요~", ephemeral=True)
                            return
    
                    for url in playlist:
                        await player.queue.put(url)
    
                    await add_interaction.response.send_message(f"선생님의 '{playlist_name}' 플레이리스트의 모든 곡을 대기열에 추가했어요!", ephemeral=True)
                except Exception as e:
                    await add_interaction.response.send_message(f"오류가 발생했어요: {str(e)}", ephemeral=True)

            async def cancel_callback(cancel_interaction):
                await cancel_interaction.response.send_message("곡 추가가 취소되었어요. ㅠㅠㅠ", ephemeral=True, delete_after=10)
    ...
    async def play(self, ctx, *, url: str):
        ...
        player = self.get_player(ctx)
        if not player:
            return await ctx.send("선생님, 음성 채널에 먼저 입장해주세요! 바다가 따라갈게요~", delete_after=10)
        ...
        if not ctx.voice_client:
            if ctx.author.voice:
                ...
                return await ctx.send("선생님, 음성 채널에 먼저 입장해주세요! 바다가 어디로 가야 할지 모르겠어요~", delete_after=10)
    ...
    async def search(self, ctx, *, query: str):
        ...
        search_message = await ctx.send("바다가 검색 중이에요. 잠시만 기다려주세요...")
        ...
        for i, entry in enumerate(filtered_entries):
            ...
            async def button_callback(interaction, entry=entry):
                ...
                await player.queue.put(entry['url'])
                await ctx.send(f"선생님, '{entry['url']}'의 노래를 재생할게요!", delete_after=10)
                if not player.is_playing:
                    await player.play_next()
    ...
    @commands.command(name='검색2', aliases=['search2'])
    async def 검색(self, ctx, *, query: str):
        ...
        searching_message = await ctx.send("바다가 열심히 검색중입니다!", delete_after=10)
        search_attempts = 0
        options = []
        seen_titles = set()

        while len(options) < 6 and search_attempts < 3:
            search_url = f"https://www.youtube.com/results?search_query={query.replace(' ', '+')}&gl=KR&hl=ko&sp=EgQQARgB"
            async with ctx.bot.session.get(search_url) as response:
                if response.status != 200:
                    return await ctx.send("검색 중 오류가 발생했어요. 나중에 다시 시도해 주세요.", delete_after=10)
            ...
            async def fetch_video_details(video_id):
                ...
                async with ctx.bot.session.get(video_url) as video_response:
                    video_html = await video_response.text()
                    video_title = re.search(r'<title>(.*?)</title>', video_html).group(1).replace(' - YouTube', '')
                    video_duration = re.search(r'"approxDurationMs":"(\d+)"', video_html)
                    video_uploader = re.search(r'"author":"(.*?)"', video_html)
                    
                    if video_duration:
                        duration_ms = int(video_duration.group(1))
                    ...
                    uploader = video_uploader.group(1) if video_uploader else "알 수 없음"

                    if video_title in seen_titles:
                        return None
                    seen_titles.add(video_title)

                    label = f"{video_title[:50]}..." if len(video_title) > 50 else video_title
                    description = f"{duration_str} - {uploader}"
                    return discord.SelectOption(label=label, description=description, value=video_url)

            tasks = [fetch_video_details(video_id) for video_id in video_ids]
            results = await asyncio.gather(*tasks)
            options.extend([result for result in results if result is not None])
            options = options[:6]  # 6개 초과 시 자름
            search_attempts += 1

        if not options:
            return await ctx.send("1분 이상 10분 이하의 동영상을 찾을 수 없어요. 다른 검색어로 시도해 주세요.", delete_after=10)

        select = Select(placeholder="재생할 곡을 선택하세요", options=options)
        ...
        embed = discord.Embed(title="검색 결과", description="재생할 곡을 선택하세요:", color=0x3498db)
        for i, option in enumerate(options):
            embed.add_field(name=f"{i+1}.", value=option.label, inline=False)
    ...
    async def 삭제(self, ctx, amount: int = 2):
        """지정된 수의 메시지를 삭제합니다."""
        if amount < 1:
            return await ctx.send("어라? 1개 이상의 메시지를 지정해 주셔야 해요. 바다가 삭제할 수 있게요!", delete_after=10)

        deleted = await ctx.channel.purge(limit=amount + 1)  # 명령어 메시지도 포함해서 삭제
        await ctx.send(f"선생님, {len(deleted)-1}개의 메시지를 깨끗하게 지웠어요! 바다가 열심히 청소했답니다~", delete_after=5)
        await self.delete_command_message(ctx)

    @삭제.error
    async def 삭제_error(self, ctx, error):
        if isinstance(error, commands.MissingPermissions):
            await ctx.send("앗, 죄송해요 선생님. 이 명령어는 특별한 권한이 필요해요. 바다가 도와드리고 싶어도 못 해드려요...", delete_after=10)
        elif isinstance(error, commands.BadArgument):
            await ctx.send("어머, 선생님? 숫자를 올바르게 입력해 주셔야 해요. 바다가 이해할 수 있게요!", delete_after=10)
        await self.delete_command_message(ctx)

    @commands.command(name='도움말', aliases=['도움', 'help'])
    async def help_command(self, ctx):
        """모든 사용 가능한 명령어와 설명을 보여줍니다."""
        embed = discord.Embed(title="바다의 명령어 목록", description="사용 가능한 모든 명령어와 설명이에요!", color=0x3498db)
    ...
    async def restart(self, ctx):
        """프로그램을 재시작합니다."""
        await ctx.send("바다가 물한모금 하고있어요! 잠시만 기다려주세요...", delete_after=5)
        
        # 봇이 다시 시작되면 사용 가능하다는 메시지를 보내기 위해 재시작 후 메시지를 보냄
        async def send_ready_message():
            await bot.wait_until_ready()
            channel = bot.get_channel(ctx.channel.id)
            if channel:
                await channel.send("바다가 다시 돌아왔어요! 이제 사용 가능해요!", delete_after=10)

        self.bot.loop.create_task(send_ready_message())
        await self.restart_program()  # 프로그램 재시작 호출
    ...
    async def stop(self, ctx):
            ...
            if ctx.voice_client:  
                await ctx.voice_client.disconnect()  # 음성 채널에서 나가기
                await ctx.send("바다가 음성 채널을 떠날게요!", delete_after=10)
                await self.restart_program()
            else:
